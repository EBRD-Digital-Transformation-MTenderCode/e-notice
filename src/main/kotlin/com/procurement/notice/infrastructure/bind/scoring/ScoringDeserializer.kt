...
import com.procurement.notice.infrastructure.exception.ScoringValueException
import java.io.IOException
import java.math.BigDecimal

class ScoringDeserializer : JsonDeserializer<BigDecimal>() {
    companion object {
        fun deserialize(text: String): BigDecimal = try {
            BigDecimal(text)
        } catch (exception: Exception) {
            throw ScoringValueException(text, exception.message ?: "")
        }
    }
    ...
    override fun deserialize(jsonParser: JsonParser, deserializationContext: DeserializationContext): BigDecimal {
        if (jsonParser.currentToken != JsonToken.VALUE_NUMBER_FLOAT && jsonParser.currentToken != JsonToken.VALUE_NUMBER_INT) {
            throw ScoringValueException(
                scoring = "\"${jsonParser.text}\"",
